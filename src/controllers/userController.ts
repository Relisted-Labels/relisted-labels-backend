...
export async function signInWithGoogle ( req: Request, accessToken: string | undefined, refreshToken: string, profile: Profile, done: VerifyCallback) {
  console.log(profile);
  ...
  if (existingUser) {
      ...
      const newUser = await User.createAccount(username, email, profile.id, isOnboarded, isEmailVerified);
      ...
      if (newUser) {
       ...
       return done('Failed to create user account', undefined);
      }
  }
}
